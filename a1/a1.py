...
def make_training_graph(graph, test_node, n):
    ...
    for i in range(n):
        tmpgraph.remove_edge(test_node, neighbors[i])
...
def jaccard(graph, node, k):
    ...
    nodeself = set()
    nodeself.add(node)
    nonneighbors = set(graph.nodes()) - set(graph.neighbors(node)) - nodeself
...
def path_score(graph, root, k, beta):
    ...
    def bfs_score(graph, root):
        ...
        return dict(sorted(n2d.items())), dict(sorted(n2n.items()))
    
    scores = []
    nodeself = set()
    nodeself.add(root)
    node2distances, node2num_paths = bfs_score(graph, root)
    nonneighbors = set(graph.nodes()) - set(graph.neighbors(root)) - nodeself
    for n in nonneighbors:
...
def main():
    ...
    """
    max_depths = [2, 3, 5]
    print(score_max_depths(example_graph(), max_depths))
    """
    ...
    train_graph = g.copy()
    train_graph.remove_edge(*('D', 'F'))
    print(path_score(train_graph, 'D', k=4, beta=.5))
    """
