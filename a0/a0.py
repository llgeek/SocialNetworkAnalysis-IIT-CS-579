...
def get_friends(twitter, screen_name):
    ...
    ids = robust_request(twitter, 'friends/ids', params).json()['ids'][:5000]
...
def friend_overlap(users):
    ...
    for i in range(len(users)):
        for j in range(i+1, len(users)):
            unionset = set(users[i]['friends']) & set(users[j]['friends'])
            t = (users[i]['screen_name'], users[j]['screen_name'], len(unionset))
            overlapuser.append(t)
    return sorted(overlapuser, key = lambda x: -x[2])
...
def followed_by_hillary_and_donald(users, twitter):
    ...
    for u in users:
        if u['screen_name'] == 'HillaryClinton':
            for s in users:
                if s['screen_name'] == 'realDonaldTrump':
                    commonid = set(u['friends']) & set(s['friends'])
    params = {'user_id' : commonid}
    commonuser = robust_request(twitter, 'users/lookup', params).json()
    return commonuser[0]['screen_name']
...
def create_graph(users, friend_counts):
    ...
    G = nx.Graph()
    for u in users:
        G.add_edges_from([u['screen_name'], fid] for fid in u['friends'] if friend_counts[fid] > 1)
    return G
...
def draw_network(graph, users, filename):
    ...
    node_labels = {u['screen_name']:u['screen_name'] for u in users}
    #labels = nx.draw_networkx_labels(graph,pos=nx.spring_layout(G), labels = node_labels)
    fig = plt.figure()
    plt.axis('off')
    G = nx.draw_networkx(graph, pos=nx.spring_layout(graph), node_size = 35, alpha = 0.45, width = 0.1, edge_color = 'k', labels = node_labels, font_size = 6)
    #G = nx.draw_networkx(graph, node_size = 50, alpha = 0.5, width = 0.4, edge_color = 'k', labels = node_labels, font_size = 8)
